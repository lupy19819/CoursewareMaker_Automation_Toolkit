...
- `purpose`：必填或生成前必须确认。可填 `diagnostic` 或 `practice`。
  ...
  - `practice`：非诊断练习模式，去掉倒计时和草稿，区分正确/错误反馈音效，关闭错误后跳关，必须答对后才能切换关卡，选项/拖拽物/填空框必须有可区分的正确错误状态，整关正确后播放撒花动效。
- `successEffect`：可选。`purpose = "practice"` 时默认应为 `fireworks`；通过 `passSuccess` / `闯关成功` 状态播放撒花 Spine 动画和成功音效。
  - `fireworks` 的具体资源和状态动作见 `data/component_skin_inventory.json` 的 `global_assets.success_fireworks` 或 `data/success_effect_assets.tsv`。
