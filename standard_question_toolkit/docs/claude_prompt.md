...
- 匹配模板或原配置后，只复用组件骨架、状态结构、答案判定和必需字段；坐标和布局必须按当前题内容重新计算。
- 不要在题干里写题号。
- 选择题使用手动判定，允许未全部作答就提交。
- 选择题选中态音效使用填空框输入中状态音效。
- 填空题统一输入框尺寸，不拉伸素材。
- 同一填空题内所有输入框 `numberUnit` 和字号一致。
- 拖拽题必须使用 `MDraggbale` + `LDragPlace`，保留 answer/judge 信息，并校验拖拽物 `answerKey` 与放置区 `accept` 的绑定。
- 行内拖拽必须拆成前文文本 + 放置区 + 后文文本，不用空格占位。
