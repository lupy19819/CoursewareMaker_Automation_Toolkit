...
| 2 | 明确给出 `game_id=`，并说 `上传`、`导入`、`保存`、`试一下` | 导入保存/跑通验证 | 显式 `game_id` 是自动保存目标；游戏名只能作为人工辅助信息，不能单独触发保存 | 新建游戏、使用同名游戏中任意一个、用 `latest_game_id.txt` 猜 |
...
- 只要用户提供了目标 `game_id` 并要求上传/导入/保存，必须使用该已有游戏；不得因为识别到游戏类型关键字而新建游戏。
- 短期规则：导入、保存、返修和发布必须显式提供 `game_id`；游戏名只能辅助人工确认，不能单独作为自动保存目标。
- 用户只给游戏名时，planner 必须 blocked 并要求补充 `game_id`。
- “资源用错、选项位置不好、放置区排版不好、音频错、正确项错”都属于返修，不属于新建。
- “试一下把这个配置上传到 game_id=...”属于导入保存验证，不属于新建。
- 如果意图不明确，先问一句确认“是修改已有 game_id 还是新建游戏”，不能自行创建。
- 进入返修环节时必须继承当前任务单或线上配置上下文；若缺少 `game_id`/配置路径，只能询问，不能用游戏名或 `latest_game_id.txt` 猜。
...
| 目标游戏解析 | `game_id`、上下文任务单、可选游戏名备注 | 唯一目标 `game_id` 和游戏详情 | 导入/保存/返修/发布必须显式 `game_id`；游戏名只作人工辅助；解析后回读 `game_name/game_type/configuration` | 游戏详情 GET、回读脚本 | 同名游戏误选、用最新创建 ID 代替用户指定名称 |
...
| 素材确认 | 本地素材、资源表、已有 URL、资源 list | 可写入配置的素材 URL 和同名确认记录 | 上传前先查同名资源；同名时先问提需用户使用现有资源还是改名上传；上传后确认真实 URL；资源 list 必须保存在 git 项目内并持续合并；运动PK生成必须使用 `resources.filtered.json` | `resources/latest_resources.json`、`courseware_bulk_upload_assets.mjs`、`scripts/sync_courseware_resources.py`、`scripts/resolve_sheet_resources.py` | 同名资源误复用、未改名上传导致混淆、URL 缺失、使用项目外旧资源表、生成器未使用 filtered resources |
...
| 配置校验 | 本地配置 JSON、当前任务单、build-meta | 分层校验报告 | 主工作流按 `validation_policy.json` 调度：规则脚本校验为主，参考配置不变量和关卡级 profile 对比为辅，案例回归用于防止脚本退化；运动PK校验必须带 build-meta，对比题目音频、题干图、选项图和正确项是否实际写入；模板游戏统一跑 `validate_template_game_config.py`，贪吃小怪兽/运动PK跑更强专项 validator；保存后统一回读比对和预览 | `workflow/workflow_planner.py`、`workflow/validation_policy.json`、`workflow/game_input_schemas.json`、`scripts/validate_yundong_pk_config.py --meta`、各专项 validator | 根结构不对、皮肤不匹配、缺组件、状态图错、题目资源 URL 未写入、模板被误判成标准拖拽题、只做案例 diff 没跑规则 |
...
| `scripts/validate_yundong_pk_config.py` | 运动PK专项校验 | 皮肤 baseline、`custom_game`；带 `--meta` 时校验题目音频、题干图、选项图和正确项写入结果 |
...
| `scripts/build_yundong_pk_config.py` | 生成运动PK配置 | Sheet 识别、run/swim/racecar baseline、读取 filtered resources、输出资源写入 build-meta；游泳按 rules 统一三态选项底图 |
