/**
 * 修复被双重序列化的游戏配置，同时更新节点_37图片
 * 关键：PUT payload.configuration 必须是解析后的对象，不能是字符串
 */
...
async function main() {
  ...
  for (const [name, gid] of games) {
    ...
    // 修复双重序列化：先parse一次得到字符串，再parse得到对象
    let rawCfg = detailMeta.configuration;
    let cfg;
    if (typeof rawCfg === 'string') {
      cfg = JSON.parse(rawCfg);
      // 如果还是字符串（双重序列化），再解一次
      if (typeof cfg === 'string') {
        console.log('Double-encoded detected! Unwrapping...');
        cfg = JSON.parse(cfg);
      }
    } else {
      cfg = rawCfg;
    ...
    console.log('Top-level keys:', Object.keys(cfg).slice(0, 5));
    console.log('game levels:', cfg.game?.length);
    ...
    // 关键：configuration 传对象（不是字符串）
    const payload = { ...detailMeta, configuration: cfg };
